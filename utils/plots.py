...
def SA_plot(file_path_all_, SA_results, indicator):
    ...
    dtP['Start avg.'] = dtP['Start avg.'].astype(str)
    dtP['Constrain'] = dtP['Constrain'].astype(str)
    dtM['Remove coil'] = dtM['Remove coil'].astype(str)
    dtM['Start avg.'] = dtM['Start avg.'].astype(str)
    ...
    variables = ['Extract', 'Samples loc.', 'Interface', 'Forward model', 'Min. method', 'Alpha', 'Remove coil', 'Start avg.', 'Constrain', 'Det. approach']
    ...
    for i, var in enumerate(variables):
        ...
        axs[i].tick_params(axis='x', labelsize=17, rotation=60)
        axs[i].xaxis.set_label_position('top')
        axs[i].set_xlabel(var, fontsize=16)  # Set xlabel to the top
        ...
        if i % num_cols != 0:
            ...
            axs[i].set_ylabel(r'Site 1. Median $RMSE$ of $\theta$', fontsize=18)
    ...
    for i, var in enumerate(variables):
        ...
        axs[i + num_cols].tick_params(axis='x', labelsize=17, rotation=60)
        ...
        if (i + num_cols) % num_cols != 0:
            ...
            axs[i + num_cols].set_ylabel(r'Site 2. Median $RMSE$ of $\theta$', fontsize=18)
...
def f6(d1, d2, targets, preds):
    ...
    plt.savefig('output_images/f6.png', dpi=600)
    plt.show()
